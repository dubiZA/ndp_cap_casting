...
Valid JWTs are included in the setup.sh file in this repo. Tokens have a maximum validity of 24 hours and these tokens in the setup.sh file are as close to that as possible. 

**NOTE: for Udacity Graders: Please use the tokens in the setup.sh file with the sample curl commands in each endpoint reference. Make sure to substitute the necessary parameters in the curl commands for the heroku URL, client_token, etc.**
